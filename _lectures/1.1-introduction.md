...
<img src="{{"assets/img/schedule.png" | relative_url }}" alt="Module schedule 2025/26">
...
<figure class="results">
    ...
    <img src="{{"assets/img/results/2024-25.png" | relative_url }}" alt="Results from 2024/25">
    <figcaption>2024/25</figcaption>
</figure>
...
subgraph somewhere else in memory
0b00000001
0b00000010
end
...
Each pyObject has an integer `id`, a pointer to a `type`, a `value`, which can be a pointer (for compound types) and a `reference count` integer.
...
We can inspect the size of a variable in memory using the `getsizeof` function from the `sys` module in the python standard library.
...
```python
getsizeof(1)        # 28
getsizeof(type(1))  # 416
```
{: .small-margin}


>The size of the integer `1` in memory is *28 bytes*.
The size of the type `<class 'int'>` is *416 bytes*.
Both are pyObjects.

We can see that larger integers take a bit more room.

```python
for i in range(20):   
    print(10**i, getsizeof(10**i))
```
...
Python was created in 1990 by *Guido van Rossum* with emphasis on code readability, its syntax allows programmers to express concepts in fewer lines of code.
...
- [![flask](https://palletsprojects.com/static/content/projects/flask-logo.svg)](https://palletsprojects.com/p/flask/)
{: .libraries}
