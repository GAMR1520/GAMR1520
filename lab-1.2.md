...
Python includes a number of compound data types used to group values together.
The most important are the `tuple`, `list`, `dictionary` and `set`.
In this set of exercises, we will explore what these data types can do for us.
...
Sequences have a length and provide efficient access to contained items using integer indices.
...
The `index` is an integer, beginning with `0` as the first character.
...
The above code returns the second last character, an 'l'.
...
message[1:-1] # 'ell'
```
The above code starts at the second character (index 1) and ends before the last character (index -1).
...
Slicing not only works with strings but also works will all python sequences including `tuples` and `lists`.
...
Strings, like integers and floats, are said to be immutable.
This means that we cannot change the value of a string (or an integer or a float).
We can only create new strings.

We can see this if we try to change a character within a string.
...
Immutability is an important concept and allows strings, integers, floats and tuples to be very simple and efficiently implemented.
...
Imagine we grew a tuple with 1,000 elements. Adding the 1001st element requires a new object to be created, leading to a doubling of the memory footprint.
...
We can also add items to a list using the
